...
class ObcHandler:
    def __init__(self, robot, intervals_handler) -> None:
        self._robot = robot
    ...
    def set_obc_state(self, state: ObcState, set_to_idle_after=0):
        ...
        if set_to_idle_after != 0:
            ...
            self._intervals_handler.add_new_interval(
                name=IntervalName.OBC_STATE.value,
                seconds=set_to_idle_after,
                is_repeating=False,
                execute_immediately=False,
                function=self._robot.subsystems.set_obc_state,
                f_args=[ObcState.IDLE],
            )
