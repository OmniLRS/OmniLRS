...
def parse_mdb_file(mdb_path):
    ...
    for enum_type in root.findall(".//xtce:EnumeratedArgumentType", XTCE_NS):
        type_name = enum_type.attrib["name"]
        mapping = {}

        enc = enum_type.find("xtce:IntegerDataEncoding", XTCE_NS)
        size_bits = int(enc.attrib.get("sizeInBits", "8")) if enc is not None else 8

        for enum in enum_type.findall(".//xtce:Enumeration", XTCE_NS):
    ...
    for int_type in root.findall(".//xtce:IntegerArgumentType", XTCE_NS):
    ...
    for bool_type in root.findall(".//xtce:BooleanArgumentType", XTCE_NS):
        type_name = bool_type.attrib["name"]

        enc = bool_type.find("xtce:IntegerDataEncoding", XTCE_NS)
        size_bits = int(enc.attrib.get("sizeInBits", "8")) if enc is not None else 8
    ...
    for float_type in root.findall(".//xtce:FloatArgumentType", XTCE_NS):
        type_name = float_type.attrib["name"]

        enc = float_type.find("xtce:FloatDataEncoding", XTCE_NS)
    ...
    for string_type in root.findall(".//xtce:StringArgumentType", XTCE_NS):
        type_name = string_type.attrib["name"]
        termination_char = None

        term = string_type.find(".//xtce:TerminationChar", XTCE_NS)
    ...
    # -------- Parse commands --------
    for cmd in root.findall(".//xtce:MetaCommand", XTCE_NS):
        cmd_name = cmd.attrib["name"]
        full_name = _build_full_command_path(cmd, parent_map)
        root_system = _get_root_system(full_name)

        # argument list
        args = []
        arg_list = cmd.find("xtce:ArgumentList", XTCE_NS)
        if arg_list is not None:
            for arg in arg_list.findall("xtce:Argument", XTCE_NS):
        ...
        # wire command name
        fixed = cmd.find(".//xtce:FixedValueEntry", XTCE_NS)
        if fixed is None:
            continue
...
def load_mdb_registry(mdb_dir):
    registry = {}
    mdb_dir = Path(mdb_dir)

    for xml_file in sorted(mdb_dir.glob("*.xml")):
        file_commands = parse_mdb_file(xml_file)
...
def decode_tc_payload(payload, registry):
    for wire_name, cmd in registry.items():
        ...
        for arg in cmd["args"]:
            ...
            if arg_type == "enum":
                raw, consumed = _decode_int(remaining, spec["size_bits"], signed=False)
                pos += consumed
                ...
                raw, consumed = _decode_int(remaining, spec["size_bits"], signed=spec["signed"])
                pos += consumed
                raw_args[arg_name] = raw
                args_out[arg_name] = raw

            elif arg_type == "float":
                raw, consumed = _decode_float(remaining, spec["size_bits"], spec["byte_order"])
                pos += consumed
                raw_args[arg_name] = raw
                args_out[arg_name] = raw

            elif arg_type == "string":
                raw, consumed = _decode_string(remaining, spec["termination_char"])
                pos += consumed
                raw_args[arg_name] = raw
                args_out[arg_name] = raw
