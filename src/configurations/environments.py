...
class Coordinates:
    ...
    def __post_init__(self):
        assert type(self.latitude) is float, "The latitude must be a float."
        assert type(self.longitude) is float, "The longitude must be a float."
...
class LunalabConf:
    ...
    def __post_init__(self):
        assert type(self.resolution) is float, "The resolution must be a float."
        assert self.resolution > 0.0, "The resolution must be greater than 0."
        assert type(self.projector_position) is tuple, "The projector position must be a tuple."
        assert len(self.projector_position) == 3, "The projector position must have 3 elements."
        assert type(self.projector_orientation) is tuple, "The projector orientation must be a tuple."
...
class LunaryardConf:
    ...
    def __post_init__(self):
        assert type(self.lab_length) is float, "The lab length must be a float."
        assert type(self.lab_width) is float, "The lab width must be a float."
        assert type(self.resolution) is float, "The resolution must be a float."
...
class LargeScaleTerrainConf:
    ...
    def __post_init__(self):
        ...
        assert type(self.profiling) is bool, "profiling must be a boolean."
        assert type(self.update_every_n_meters) is float, "update_every_n_meters must be a float."
        ...
        assert type(self.block_size) is int, "block_size must be an integer."
        assert self.block_size > 0, "block_size must be a positive integer."
        assert type(self.dbs_max_elements) is int, "dbs_max_elements must be an integer."
        assert self.dbs_max_elements > 0, "dbs_max_elements must be a positive integer."
        assert type(self.dbs_save_to_disk) is bool, "dbs_save_to_disk must be a boolean."
        assert type(self.dbs_write_interval) is int, "dbs_write_interval must be an integer."
        assert self.dbs_write_interval > 0, "dbs_write_interval must be a positive integer."
        assert type(self.hr_dem_resolution) is float, "hr_dem_resolution must be a float."
        assert self.hr_dem_resolution > 0, "hr_dem_resolution must be a positive float."
        assert type(self.hr_dem_generate_craters) is bool, "hr_dem_generate_craters must be a boolean."
        assert type(self.hr_dem_num_blocks) is int, "hr_dem_num_blocks must be an integer."
        ...
        if self.crater_gen_seed is None:
            ...
            assert type(self.crater_gen_seed) is int, "crater_gen_seed must be an integer."
        ...
        assert type(self.crater_gen_densities) is list, "crater_gen_densities must be a list."
        assert len(self.crater_gen_densities) > 0, "crater_gen_densities must have at least one element."
        assert all([type(d) is float for d in self.crater_gen_densities]), (
        ...
        assert type(self.crater_gen_radius) is list, "crater_gen_radius must be a list."
        assert len(self.crater_gen_radius) > 0, "crater_gen_radius must have at least one element."
        assert all(type(r) is list for r in self.crater_gen_radius), "crater_gen_radius must be a list of list."
        ...
        assert type(self.crater_gen_profiles_path) is str, "crater_gen_profiles_path must be a string."
        assert os.path.exists(self.crater_gen_profiles_path), "crater_gen_profiles_path does not exist."
        assert type(self.crater_gen_padding) is float, "crater_gen_padding must be a float."
        assert self.crater_gen_padding > 0, "crater_gen_padding must be a positive float."
        assert type(self.crater_gen_min_xy_ratio) is float, "crater_gen_min_xy_ratio must be a float."
        assert type(self.crater_gen_max_xy_ratio) is float, "crater_gen_max_xy_ratio must be a float."
        ...
        assert type(self.crater_gen_random_rotation) is bool, "crater_gen_random_rotation must be a boolean."
        assert type(self.crater_gen_num_unique_profiles) is int, (
        ...
        if self.crater_gen_metadata_seed is None:
            ...
            assert type(self.crater_gen_metadata_seed) is int, "crater_gen_metadata_seed must be an integer."
        ...
        if self.crater_gen_distribution_seed is None:
            ...
            assert type(self.crater_gen_distribution_seed) is int, "crater_gen_distribution_seed must be an integer."
        ...
        assert type(self.hrdem_interpolation_method) is str, "hrdem_interpolation_method must be a string."
        assert self.hrdem_interpolation_method in [
            "nearest",
            "bilinear",
            "bicubic",
        ], "hrdem_interpolation_method must be either 'nearest', 'bilinear' or 'bicubic'."
        assert type(self.hrdem_interpolator_name) is str, "hrdem_interpolator_name must be a string."
        assert self.hrdem_interpolator_name in [
            "PIL",
            "OpenCV",
        ], "hrdem_interpolator_name must be either 'PIL' or 'OpenCV'."
        assert type(self.hrdem_interpolator_padding) is int, "hrdem_interpolator_padding must be an integer."
        ...
        assert type(self.num_workers_craters) is int, "num_workers_craters must be an integer."
        assert self.num_workers_craters > 0, "num_workers_craters must be a positive integer."
        assert type(self.num_workers_interpolation) is int, "num_workers_interpolation must be an integer."
        assert self.num_workers_interpolation > 0, "num_workers_interpolation must be a positive integer."
        assert type(self.input_queue_size) is int, "input_queue_size must be an integer."
        assert self.input_queue_size > 0, "input_queue_size must be a positive integer."
        assert type(self.output_queue_size) is int, "output_queue_size must be an integer."
        ...
        assert type(self.lr_dem_folder_path) is str, "lr_dem_folder_path must be a string."
        ...
        assert type(self.geo_cm_num_texels_per_level) is int, "geo_cm_num_texels_per_level must be an integer."
        assert self.geo_cm_num_texels_per_level > 0, "geo_cm_num_texels_per_level must be a positive integer."
        assert type(self.geo_cm_target_res) is float, "geo_cm_target_res must be a float."
        assert self.geo_cm_target_res > 0, "geo_cm_target_res must be a positive float."
        assert type(self.geo_cm_fine_interpolation_method) is str, "geo_cm_fine_interpolation_method must be a string."
        assert self.geo_cm_fine_interpolation_method in [
            "bilinear",
            "bicubic",
        ], "geo_cm_fine_interpolation_method must be either 'bilinear' or 'bicubic'."
        assert type(self.geo_cm_coarse_interpolation_method) is str, (
        ...
        assert type(self.geo_cm_fine_acceleration_mode) is str, "geo_cm_fine_acceleration_mode must be a string."
        assert self.geo_cm_fine_acceleration_mode in [
            "hybrid",
            "gpu",
        ], "geo_cm_fine_acceleration_mode must be either 'hybrid' or 'gpu'."
        assert type(self.geo_cm_coarse_acceleration_mode) is str, "geo_cm_coarse_acceleration_mode must be a string."
        assert self.geo_cm_coarse_acceleration_mode in [
            "hybrid",
            "gpu",
        ], "geo_cm_coarse_acceleration_mode must be either 'hybrid' or 'gpu'."
        assert type(self.geo_cm_apply_smooth_shading) is bool, "geo_cm_apply_smooth_shading must be a boolean."
        ...
        assert type(self.terrain_collider_resolution) is float, "terrain_collider_resolution must be a float."
        assert self.terrain_collider_resolution > 0, "terrain_collider_resolution must be a positive float."
        assert type(self.terrain_collider_mode) is str, "terrain_collider_mode must be a string."
        assert type(self.terrain_collider_cache_size) is int, "terrain_collider_cache_size must be an integer."
        assert self.terrain_collider_cache_size >= 4, "terrain_collider_cache_size must be greater or equal to 4."
        assert type(self.terrain_collider_building_threshold) is float, (
        ...
        if self.rock_gen_main_seed is None:
            ...
            assert type(self.rock_gen_main_seed) is int, "rock_gen_main_seed must be an integer."
        ...
        self.sun_path: str = "/LargeScaleLunar/Sun"
